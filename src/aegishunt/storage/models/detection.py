...
class AlertGroupRecord(Base):
    ...
    alert_count: Mapped[int | None] = mapped_column(nullable=True)
    severity: Mapped[Severity | None] = mapped_column(
        string_enum(Severity, name="group_severity"), nullable=True, index=True
    )
    ...
    group_schema_version: Mapped[str | None] = mapped_column(String(64), nullable=True)
    created_at: Mapped[datetime | None] = mapped_column(UTCDateTime(), nullable=True, index=True)
