...
- [ ] Working tree is clean at push.
- [ ] Phase 14 PR is open against `main`; no force push or direct main push.

## Code and tests

- [ ] Ruff, strict mypy, full pytest, branch coverage >=85%.
- [ ] All 17 Phase 13 core packages remain >=80%.
- [ ] Offline E2E, full final demo, persistence/restart, security, robustness,
  and performance smoke pass.
- [ ] No test is deleted, skipped, xfailed, or weakened to hide a defect.

## Packaging

- [ ] Wheel and sdist build; Twine metadata check passes.
- [ ] Python 3.11 and 3.12 fresh wheel installs use no editable mode or
  `PYTHONPATH`; `pip check`, CLI, doctor, DB, API, frontend, worker, demo pass.
- [ ] Package/image inventory has no secret, DB, formal model, cache, or local
  path.

## Docker

- [ ] Image builds from wheel and runs as non-root UID 10001.
...
- [ ] No DB, generated release bundle, formal model binary, upload, raw scan,
...
- [ ] Ignored release bundle builds without overwrite and its exact manifest
