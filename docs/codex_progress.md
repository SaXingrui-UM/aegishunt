...
| Current activity | The bounded corrective on `codex/runtime-job-provenance-filters` resolves replay creation/execution against one pinned artifact environment; reports the actual manual-worker claim outcome and associated runtime job status; prevents downstream correlation and hypothesis generation from consuming unrelated retained evidence; restores runtime-pinned Native/Permutation feature importance; and adds one persistent runtime `job_id` scope to Traffic Explorer, Alerts, Threat Hunts, Cases, and their list APIs. It is not Phase 15 and does not alter trained model content, thresholds, fusion/risk policy, historical outputs, or the canonical Phase 14 Tag |
| Corrective status | Implementation complete — awaiting PR review |
| Verification status | Merged-main Ruff passed; strict mypy passed 239 source files; all 550 pytest tests passed with 0 failed/skipped/xfailed at 85.83% branch-aware coverage; every one of the 17 declared core packages exceeds 80%. External Python 3.11 and 3.12 wheel installs passed without editable mode or `PYTHONPATH`. Wheel/sdist, Twine metadata, exact distribution inventory, documentation delivery, and release-bundle verification passed. Security, robustness, and performance gates passed; the formal final Codex Security rescan remains explicitly waived, was not executed, and the complementary controls are not represented as a substitute. A fresh local arm64 no-cache Docker build passed after an initial historical Docker Hub metadata timeout. The final non-root/read-only Compose run passed health/OpenAPI/Streamlit, `doctor`, a 1,017-packet/42-flow controlled demo, 42 alerts, one group, one hypothesis, case/note/verdict/feedback, restart persistence, schema v5/WAL, and SQLite integrity `ok`. Matplotlib now uses explicit writable `/tmp/matplotlib`; no read-only-home warning remains. Corrective verification passed Ruff, strict mypy for 239 source files, and all 564 pytest tests with 0 failures at 85.91% branch-aware coverage. The worker-timeout regression uses a dedicated 600-second bound while keeping ordinary API requests at 15 seconds. A real 42,888,106-byte PCAP worker call remained connected for 96.13 seconds and completed with 449 flows, 449 detections, 449 alerts, 94 groups, and 94 hypotheses. Separate preflight runs completed both supplied PCAPs: the 2025 capture produced 449 flows/alerts and 94 hypotheses, while the 2026 capture produced 1,170 flows/alerts and 337 hypotheses. The Docker artifact follow-up passed Ruff, strict mypy for 239 source files, and 48 relevant unit/integration/frontend tests. The preserved `demo-v2` export downloaded as a verified 1,807-byte ZIP containing its exact four-file inventory. The replay-statistics/evidence-clarity corrective passed Ruff, strict mypy for 240 source files, and all 570 pytest tests with zero failures at 85.95% branch-aware coverage; rendered Browser QA used both retained PCAP replay jobs and verified the Case replacement guard and both importance modes. The runtime-provenance corrective passed Ruff, strict mypy for 242 source files, and all 583 pytest tests with zero failures at 85.93% branch-aware coverage; rebuilt local Compose services were healthy, rendered browser QA produced no console errors, and retained jobs were isolated as 42 flows/alerts with one group/hypothesis/case and 1,170 flows/alerts with 337 groups/hypotheses and no case |
...
| Pull requests | Phase 5 PRs #13–#17, Phase 6 PRs #18–#20, Phase 7 PRs #21–#24, Phase 8 PRs #25–#27, Phase 9 PRs #28–#29, Phase 10 PR #31, Phase 11 PR #33, Phase 12 PR #35, validation PR #37, corrective PR #38, Phase 13 PR #39, Phase 14 PR [#41](https://github.com/SaXingrui-UM/aegishunt/pull/41), mentor-demo corrective PR [#43](https://github.com/SaXingrui-UM/aegishunt/pull/43), Docker/evidence corrective PR [#44](https://github.com/SaXingrui-UM/aegishunt/pull/44), and historical-evidence corrective PR [#46](https://github.com/SaXingrui-UM/aegishunt/pull/46) are merged |
...
| Next action | Review and merge the bounded runtime-provenance corrective after its required checks pass. Do not create another post-merge checkpoint PR and do not move or replace `phase-14-complete` |
