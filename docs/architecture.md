...
Phase 14 packages the same modular monolith as application version `1.0.0`,
adds a non-root local Compose topology, exact release manifests, source-backed
final evidence, and thesis materials without changing any model or research
selection.
...
| delivery | Wheel, local Compose, release manifest, documentation and thesis evidence | 14 |
...
Local Python and Docker Compose are supported final-delivery modes. Default
host listeners bind to loopback. Inputs cross an untrusted
...
## Final deployment views

```mermaid
flowchart LR
    Browser["Local browser"] -->|"127.0.0.1:8501"| ST["Streamlit"]
    ST -->|"127.0.0.1:8000"| FA["FastAPI"]
    CLI["Typer CLI"] --> Core["Typed services"]
    FA --> Core
    Worker["One runtime worker"] --> Core
    Core --> SQLite["SQLite WAL"]
    Core --> Files["Bounded data / verified artifacts / reports"]
```

```mermaid
flowchart TB
    Host["Host loopback"] -->|"127.0.0.1:8000"| API["api · UID 10001"]
    Host -->|"127.0.0.1:8501"| Frontend["frontend · UID 10001"]
    Frontend -->|"http://api:8000 private bridge"| API
    Init["init · idempotent"] --> Data[("data volume")]
    API --> Data
    Worker["worker · one process"] --> Data
    API --> Artifacts[("artifact volume")]
    Worker --> Artifacts
    API --> Reports[("report volume")]
```

Compose containers have read-only roots, drop capabilities, enable
`no-new-privileges`, and receive only declared named volumes. They do not mount
the Docker socket, host network, home, SSH, or Git directory. `init` only
validates/initializes storage; demo, training, and activation remain explicit.

## Final runtime and analyst sequences

```mermaid
sequenceDiagram
    participant O as Operator
    participant A as API/CLI
    participant J as RuntimeJob service
    participant W as Worker
    participant S as SQLite/artifacts
    O->>A: Explicit replay of completed PCAP source
    A->>J: Verify source and pinned artifact identities
    J->>S: Commit queued job
    W->>J: Claim lease and repeat preflight
    W->>S: Transactional flow/detection/optional-alert batches
    W->>S: EOF flush, correlation, hypothesis, completion
    A-->>O: Live observed + durable committed progress
```

```mermaid
sequenceDiagram
    participant D as Detection
    participant R as Risk/explanation
    participant C as Correlation/hypothesis
    participant I as Investigation case
    participant H as Analyst
    D->>R: Identity-bound engine/fusion evidence
    R->>C: Optional threshold-gated alert
    C->>I: Proposed evidence-rich hypothesis
    H->>I: Case, note, verdict, feedback
    I-->>H: Audited report and review-only candidate
```
...
```mermaid
flowchart LR
    Bytes["Untrusted or generated bytes"] --> Boundary["Root containment + no symlink"]
    Boundary --> Inventory["Exact inventory + type allowlist"]
    Inventory --> Hash["SHA-256 + identity/schema versions"]
    Hash --> Use["Typed load / transaction"]
    Use --> Evidence["Immutable formal evidence or explicit demo namespace"]
```

Selection, frozen test, demo, and performance evidence remain separate. The
release directory refuses overwrite, missing, extra, linked, corrupt, and
escaped files. Formal model workspaces are not copied into the image.

Input/artifact failure is sanitized and fail closed. Output batches and ledgers
commit atomically. Interrupted replay becomes `recovery_pending`; explicit
recovery restarts from packet zero and does not claim exact-cursor resume.
Repository writes append audits inside the transaction. Application `1.0.0`,
database schema `5`, feature schema `1.0.0`, and every dataset/model/policy or
artifact identity remain independent.

This is local, single-user, loopback-first, one-worker SQLite research
software. Docker is not a production isolation guarantee. Authentication,
RBAC, TLS, distributed data/control planes, live capture, automatic response or
retraining, remote immutable audit, orchestration, and high availability remain
